...
class Payroll(db.Model):
    ...
    payroll_month = db.Column(
        db.String(20),
        nullable=False
    )

    payroll_year = db.Column(
        db.Integer,
        nullable=False
    )
...
class KitchenItem(db.Model):
    ...
    category_id = db.Column(
        db.Integer,
        db.ForeignKey("kitchen_categories.id"),
        nullable=False
    )
...
class KitchenIssue(db.Model):
    __tablename__ = "kitchen_issues"
...
class MealServing(db.Model):
    ...
    menu_id = db.Column(
        db.Integer,
        db.ForeignKey("meal_menus.id"),
        nullable=False
    )

    serving_date = db.Column(
        db.Date,
        default=date.today,
        nullable=False
    )
...
class KitchenConsumption(db.Model):
    __tablename__ = "kitchen_consumption"
...
def dashboard():
    ...
    inactive_pupils = Pupil.query.filter_by(
        school_id=school_id,
        status="Inactive"
    ).count()
    ...
    for pupil in active_pupil_records:
        ...
        pupil_balance = (
            float(pupil_due or 0)
            - float(pupil_paid or 0)
            - float(pupil_discount or 0)
        )
...
def notify_exam_results():
    ...
    if grade not in GRADES:
        flash("Please select a valid grade.")
        return redirect(url_for("report_cards"))
    ...
    for pupil in pupils:
        ...
        #
        # RULE:
        # The pupil name must NEVER disappear.
        #
        # We shorten other wording first.
        # If still necessary, the pupil name itself is
        # shortened, but never completely removed.
        # =====================================================

        # -----------------------------------------------------
        # VERSION 1
        # Full pupil + full exam name
        # -----------------------------------------------------
        sms_message = (
        ...
        # -----------------------------------------------------
        # VERSION 2
        # Shorter wording, full pupil + exam
        # -----------------------------------------------------
        if not segment_info["fits"]:

            sms_message = (
                f"Results: {pupil_name}. "
                f"{exam_name} results ready. "
                f"View report card or contact school."
            )
        ...
        # -----------------------------------------------------
        # VERSION 4
        # Keep pupil name, remove exam name if necessary
        # -----------------------------------------------------
        if not segment_info["fits"]:

            sms_message = (
                f"Results for {pupil_name} "
                f"are ready. "
                f"View report card or contact school."
            )
        ...
        # -----------------------------------------------------
        # VERSION 5
        # Shorten pupil name, but NEVER remove it
        # -----------------------------------------------------
        if not segment_info["fits"]:
            ...
            sms_message = (
                f"{short_name}: exam results "
                f"are ready. View report card."
            )
        ...
        #
        # Important:
        # Do NOT identify duplicates only by phone.
        #
        # Two siblings may share the same guardian phone.
        # Each pupil must still receive their own result alert.
        # =====================================================
        duplicate_sms = (
            SMSMessage.query
            .filter(
        ...
        if (
            ...
            try:
                ...
                print(
                    "EXAM SMS QUEUE ERROR:",
                    pupil_name,
                    str(sms_error),
                    flush=True
                )

        elif not segment_info["fits"]:
            sms_failed += 1
    ...
    try:
        ...
        print(
            "EXAM ALERT COMMIT ERROR:",
            str(error),
            flush=True
        )

        flash(
            "Exam alerts could not be completed."
        )
...
def payments():
    ...
    if request.method == "POST":
        try:
            ...
            #
            # Very important:
            # Once the payment is valid, save it BEFORE
            # attempting WhatsApp or SMS.
            #
            # Communication failure must never cancel a
            ...
            # =================================================
            # DETAILED WHATSAPP CONFIRMATION
            # =================================================
            #
            # WhatsApp does not need the one-SMS restriction.
            # =================================================
            ...
            if pupil.guardian_phone:
                ...
                # =============================================
                #
                # Pupil name is mandatory.
                # It is NEVER completely removed.
                # =============================================
                pupil_name = (
                    pupil.full_name
                    or "Pupil"
                ...
                # ---------------------------------------------
                # VERSION 2
                # Shorter wording, full pupil name
                # ---------------------------------------------
                if not segment_info["fits"]:

                    sms_message = (
                        f"Payment: {pupil_name}. "
                        f"KES {amount_paid:,.0f} "
                        f"received. "
                        f"Rcpt {receipt_number}. "
                        f"Bal KES "
                        f"{balance:,.0f}."
                ...
                # ---------------------------------------------
                # VERSION 5
                # Compact, still keeps pupil name
                # ---------------------------------------------
                if not segment_info["fits"]:
                ...
                # FINAL DEFENSIVE FALLBACK
                #
                # Drop receipt number only if absolutely
                # necessary, but retain pupil identity.
                # ---------------------------------------------
                if not segment_info["fits"]:
...
@app.route("/staff_hr/<int:staff_id>", methods=["GET", "POST"])
def staff_hr(staff_id):
    if not login_required():
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        hr.department = request.form.get("department", "")
        hr.employment_type = request.form.get("employment_type", "")
        hr.employment_status = request.form.get("employment_status", "")

        hr.basic_salary = float(request.form.get("basic_salary") or 0)
        hr.house_allowance = float(request.form.get("house_allowance") or 0)
        hr.transport_allowance = float(request.form.get("transport_allowance") or 0)
        hr.other_allowance = float(request.form.get("other_allowance") or 0)

        hr.kra_pin = request.form.get("kra_pin", "")
        hr.nhif_number = request.form.get("nhif_number", "")
        hr.nssf_number = request.form.get("nssf_number", "")

        hr.payment_mode = request.form.get("payment_mode", "")
        hr.bank_name = request.form.get("bank_name", "")
        hr.account_number = request.form.get("account_number", "")
        hr.mpesa_number = request.form.get("mpesa_number", "")

        db.session.commit()
        flash("HR profile updated successfully.")
        return redirect(url_for("staff_hr", staff_id=staff.id))
...
def whatsapp_settings():
    ...
    if request.method == "POST":
        school.whatsapp_enabled = request.form.get("whatsapp_enabled") == "on"
        school.whatsapp_sender_name = request.form.get("whatsapp_sender_name", "")
        school.whatsapp_access_token = request.form.get("whatsapp_access_token", "")
        school.whatsapp_phone_number_id = request.form.get("whatsapp_phone_number_id", "")
        school.whatsapp_business_account_id = request.form.get("whatsapp_business_account_id", "")
...
@app.route("/kitchen/consumption", methods=["GET", "POST"])
def kitchen_consumption():
    ...
    if request.method == "POST":
        ...
        save_audit(
            f"Recorded kitchen consumption: "
            f"{quantity} {item.unit} of {item.name}",
            "Kitchen"
        )
    ...
    records = KitchenConsumption.query.filter_by(
        ...
        KitchenConsumption.created_at.desc()
    ).limit(200).all()

    return render_template(
        "kitchen/consumption.html",
        settings=get_settings(),
        items=items,
        menus=menus,
        records=records,
...
@app.route("/kitchen/serving", methods=["GET", "POST"])
def meal_serving():
    if not login_required():
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        ...
        save_audit(
            f"Recorded {total_served} meals served "
            f"for {serving.meal_type}",
            "Kitchen"
        )
    ...
    servings = MealServing.query.filter_by(
        ...
        MealServing.created_at.desc()
    ).limit(200).all()

    return render_template(
        "kitchen/serving.html",
        settings=get_settings(),
        menus=menus,
        servings=servings,
        today=date.today()
...
def kitchen_reports():
    ...
    consumptions = KitchenConsumption.query.filter_by(
        school_id=school_id
    ).all()
...
def verify_mobile_token(token, max_age=60 * 60 * 24 * 30):
    serializer = URLSafeTimedSerializer(app.secret_key)
